...
#if defined(NDEBUG)
# error "Prototanium cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Prototanium Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (!CheckBlockProofOfWork(&block, (chainActive.Tip() != NULL ? chainActive.Tip()->nHeight : 0)))
    ...
    if (!IsInitialBlockDownload() && !fWarned)
    {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++)
        {
			if (pindex->nVersion > CBlock::CURRENT_VERSION &&
				(pindex->nVersion & ~BLOCK_VERSION_AUXPOW) != (CBlockHeader::CURRENT_VERSION | ((pindex->nHeight >= 700000 ? AUXPOW_CHAIN_ID_FORK : AUXPOW_CHAIN_ID) * BLOCK_VERSION_CHAIN_START)))
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckBlockProofOfWork(&block, (chainActive.Tip() != NULL ? chainActive.Tip()->nHeight : 0)))
