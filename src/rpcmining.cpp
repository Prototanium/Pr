...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().MineBlocksOnDemand())
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            while (!CheckBlockProofOfWork(pblock, chainActive.Tip()->nHeight)) {
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Prototanium is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Prototanium is downloading blocks...");
...
Value getauxblock(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Prototanium is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Prototanium is downloading blocks...");
